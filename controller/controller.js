...
async function SearchVideo(req,res){
    ...
    const url= `${baseApiUrl}/search?key=${apiKey}&type=video&part=snippet&q=${searchQuery}&maxResults=27`;
...
async function Category(req,res){
    ...
    const url = `${baseApiUrl}/videos?part=snippet&regionCode=in&maxResults=27&videoCategoryId=${videoId}&chart=mostPopular&key=${apiKey}`;
    const response = await axios.get(url);
    //const titles= response.data.items.map((item)=>item.snippet.title);
    res.send(response.data.items);
    //res.send(titles);
